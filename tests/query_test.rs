...
#[test]
fn test_query_exits_cleanly_on_broken_pipe() {
    let dir = create_query_test_repo();
    let mut child = Command::new(env!("CARGO_BIN_EXE_tracegrep"))
        .arg("validate_body")
        .current_dir(dir.path())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
#[test]
fn test_query_enriches_test_file_matches_without_include_tests() {
    let dir = create_repo(&[(
...
"#,
    ...
    assert!(
        stdout.contains("tests/integration.rs:helper"),
        "stdout:\n{stdout}"
    );
    assert!(stdout.contains("Called by tests:"), "stdout:\n{stdout}");
    assert!(stdout.contains("test_helper"), "stdout:\n{stdout}");
}
